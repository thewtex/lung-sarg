...
import { FeatureViewId, appContext } from './state/app.machine.js';
...
export class ScanViews extends LitElement {
  ...
  private addHandler() {
    this.stateService.value?.service.send({ type: 'FEATURE_ADD' });
  }
    ...
    return html`
      <div class="feature-grid">
        ${repeat(
          ...
          ([id]) => id,
          ([id, feature]) => {
            return html`<feature-scans
              .feature=${feature}
              @feature-close=${this.featureCloseHandler(id)}
              @featureSelected=${this.valueChangedHandler(id)}
            ></feature-scans>`;
          },
        )}
        <div
